...
export const SelectEndIcon = styled.div<ISelectEndIconProps>`
  ...
  ${({ opened }) =>
    ...
    `}
`;
interface IFormatSelectOption {
  [key: string]: string;
}
export interface ISelectProps {
  options: IFormatSelectOption;
  selected: "0" | "asc" | "desc";
  onChange: (option: "0" | "asc" | "desc") => void;
  label?: string;
...
export const Select: FC<ISelectProps> = ({
  ...
  const selectOption = (option: "0" | "asc" | "desc") => {
    onChange(option);
    closeMenu();
  };
      ...
      >
        {Object.keys(options).map((o) => {
          return (
            <SelectItem
              key={o}
              selected={options[o] === options[selected]}
              onClick={() => selectOption(o as "0" | "asc" | "desc")}
            >
              {options[o]}
            </SelectItem>
          );
